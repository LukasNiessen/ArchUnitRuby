...
**ArchUnitRuby** is the Ruby member of the ArchUnitEverything family: one architecture-testing library
...
production, plus ArchUnitGo, ArchUnitNET and ArchUnitRust, which are being built alongside this one.
...
**Follow it by default. Where Ruby makes a convention awkward, diverge — knowingly, and leave a
line in the commit message or a comment saying why.** Fighting the host language to match a sibling
is the one failure mode worse than divergence. Idiomatic Ruby wins whenever the two genuinely
...
| Stage | Does what | Ruby-specific? |
|---|---|---|
| **Source** | The user's project, found from a project root or a build file. | Only in how the root is found. |
| **Extract** | Walk and parse the source, resolve every import to a target, emit `Edge` values. | **Yes, entirely.** This is the only stage that knows Ruby. |
...
The consequence worth internalising: **almost all the Ruby-specific work is in Extract.** If you
...
```
lib/archunit/common/         the kernel — everything shared
lib/archunit/  extraction/   the dependency-graph extractor  <- the only Ruby-specific code
lib/archunit/  projection/   reshaping the graph, plus cycles/
lib/archunit/  assertion/    Violation, EmptyTestViolation
lib/archunit/  fluentapi/    Checkable, CheckOptions
lib/archunit/  error/        TechnicalError, UserError
lib/archunit/  util/         logging, pattern matching, path helpers
lib/archunit/files/          file-level dependency and naming rules
lib/archunit/layers/         named-layer policy
lib/archunit/slices/         component and diagram rules
lib/archunit/metrics/        numeric code-quality rules
lib/archunit/graph/          dependency-graph reports
lib/archunit/testing/        violation formatting and test-framework glue
lib/archunit.rb              the public surface — re-exports, nothing else
```
...
1. `common` depends on nothing but the standard library and the Ruby analysis toolchain.
...
Six types carry everything. Keep the names; adapt the shapes to whatever Ruby wants.
...
- **Zero matches is a violation, not a pass.** A selector matching no files is almost always a typo,
  ...
  terminal needs it.
- `ImportKind` is **expected to differ per language**. Here it is `:require`, `:require_relative`, `:autoload`, `:load`. This is the one
...
architect who does not write Ruby would understand, the name is wrong.** Not the implementation —
the name.
...
Word choice is fixed. Casing is yours. Methods and files are `snake_case`, classes and modules `CamelCase`. `depend on files` becomes `depend_on_files`.
...
| Files | One concept per file, named after the concept: `extract_graph.rb`, with the spec at `spec/common/extraction/extract_graph_spec.rb` |
...
- **Gem** `archunit`, code under `lib/archunit/`. Directories mirror the module tree:
  `lib/archunit/common/extraction/extract_graph.rb` defines `ArchUnit::Common::Extraction`.
  `fluentapi` stays one word, so the module is `FluentApi`.
- **Use Ruby's parentheses-free reading.** `should` and `should_not` are argument-less methods, so
  the chain reads as a sentence with no punctuation noise. That is a real advantage over the
  siblings — take it. Mood is never a symbol argument.
- **Extraction** uses Prism, the official Ruby parser. Locate the project by `Gemfile` or
  `*.gemspec`, defaulting to the working directory.
- **Be honest about dynamic requires.** Ruby resolves a lot at runtime. Record what can be resolved
  statically, and document plainly what cannot, rather than guessing.
- **Options** are keyword arguments with defaults, collected into a `CheckOptions` value object for
  `check`.
- **Errors** are `ArchUnit::TechnicalError` and `ArchUnit::UserError`, both `< StandardError`.
  A failing rule is a returned array — never a raise.
- **Testing.** RSpec is the target: a `pass` matcher used as `expect(rule).to pass`, registered on
  load when `RSpec` is defined and silently skipped when it is not. Minitest gets
  `assert_passes(rule)`.
- **Node vocabulary.** Files stay primary. Ruby constants often span files; do not try to model the
  constant graph in the first pass.
- `# frozen_string_literal: true` everywhere, no monkey-patching of core classes, RuboCop clean.
...
- Issues in this repository cover a large chunk of the planned work. They are starting points, not
  specifications. If one conflicts with reality in Ruby, say so and we adjust it.
